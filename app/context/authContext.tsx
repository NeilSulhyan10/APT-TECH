...
export interface UserData {
  ...
  bio?: string | null;
  // Add other fields you store in Firestore, e.g., college, year_of_study
  college?: string | null;
...
export const AuthProvider: React.FC<AuthProviderProps> = ({ children }) => {
  ...
  const pathname = usePathname(); // <--- NEW: Initialize usePathname
  ...
  const fetchUserProfile = async (user: User | null) => {
    ...
    try {
      ...
      if (userDocSnap.exists()) {
        ...
        // Fix 3: Include firstName and lastName in localStorage
        localStorage.setItem(
          ...
          JSON.stringify({
            ...
            lastName: profileData.lastName,   // <-- Include lastName
            // Include other critical data if needed, e.g., isExpertApproved
        ...
        // FIX 1: DO NOT signOut if Firestore profile is not immediately found.
        // Instead, set a default userData and allow them to remain authenticated.
        setUserData({
          uid: user.uid,
        ...
        // You might want to eventually redirect to a "complete your profile" page
        // if critical data is missing, instead of just defaulting.
      }
    } catch (error) {
      ...
      localStorage.removeItem("userInfo");
      // Do NOT sign out here automatically. The Firebase Auth session is still valid.
      // Show an error in the UI or ask user to refresh.
    } finally {
      setLoading(false);
    }
  };
  ...
  useEffect(() => {
    const unsubscribe = onAuthStateChanged(auth, async (user) => {
      ...
      // Set loading to true immediately when auth state changes to fetch new profile
      setLoading(true);
  ...
  // FIX 2: NEW useEffect for role-based redirection
  useEffect(() => {
    // Redirect only if:
    ...
    if (!loading && userData) {
      ...
      // 'student' role (from your interface, which maps to 'user' in register.tsx) defaults to '/'

      // If user is on a login/register page AND they are now authenticated, redirect them
      if (isAuthenticatedPage) {
        console.log(`Redirecting authenticated ${currentRole} from ${pathname} to ${targetPath}`);
        router.push(targetPath);
      }
      // You might add additional logic here for unauthorized access to specific routes
      // e.g., if a student navigates to '/admin/dashboard', redirect them to '/'
      // Example:
      // else if (currentRole === 'student' && pathname.startsWith('/admin')) {
      //   router.push('/');
      // }

    } else if (!loading && !userData && pathname !== '/login' && pathname !== '/register') {
  ...
  }, [loading, userData, router, pathname]); // Dependencies for this effect
  ...
  const isAuthenticated = !!userData; // isAuthenticated depends on fetched userData
